package be.intecbrussel.eenmaligSorteren;
//EENMALIG SORTEREN:
...
public class Opdracht8 {
    public static void main(String[] args) {
        ...
        patientList.sort(Comparator.naturalOrder()); // 1st method
        for (Patient p : patientList) {
            System.out.println(p); /*result: alles afprinted alfabetisch starts
            met Aviva eindt met Yohana*/
        }


       System.out.println("-------");

patientList.sort(Comparator.comparingInt(Patient::getAge)); //2d method
        System.out.println("-----------");

        for(Patient p:patientList){
            System.out.println(p);/*RESULT: alles afrpinted op basis van leeftijd
            begint met Yana age=3 eindt met Aviva age=113*/
        }
